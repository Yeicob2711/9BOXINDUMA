...
# --- Localizador robusto del Excel ---
def _resolve_excel_path() -> Path | None:
    # Candidatos comunes: mismo directorio del script, subcarpeta data/
    base = Path(__file__).parent
    candidates = [
        base / "Tactico_9box (1).xlsx",
        base / "Tactico_9box.xlsx",
        base / "data" / "Tactico_9box (1).xlsx",
        base / "data" / "Tactico_9box.xlsx",
    ]
    for p in candidates:
        if p.exists():
            return p
...
def load_data():
    ...
    if excel_file is None:
        st.error("No encuentro el archivo Excel en el repositorio. Sube el archivo o renómbralo a 'Tactico_9box (1).xlsx' y vuelve a ejecutar.")
        # Opción de carga manual
        uploaded = st.file_uploader("Sube el Excel Táctico 9-Box", type=["xlsx"])
        if uploaded is None:
            st.stop()
        excel_file = uploaded
